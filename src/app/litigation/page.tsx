...
export default function LitigationPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (intake)
        setUploadsCount((JSON.parse(intake).uploads?.length as number) || 0);
    } catch {}
  ...
  const daysToSOL = useMemo(() => {
    ...
    const diff = Math.ceil(
      (sol.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)
    );
    return diff;
  }, [lit.solDate]);

  const recommend = useMemo(() => {
    const lowOfferTrigger = !!(
      lit.targetAmount &&
      bestInsurer &&
      bestInsurer.amount < lit.targetAmount * 0.7
    );
    const solTrigger = daysToSOL != null && daysToSOL <= 30;
    const explicitReasons =
      lit.considerReasons.disputedLiability ||
      lit.considerReasons.coverageIssues ||
      lit.considerReasons.clientPreference;
    return (
      lowOfferTrigger ||
      solTrigger ||
      explicitReasons ||
      lit.considerReasons.lowOffer
    );
    ...
    <div className="w-full p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Review options and, if needed, refer to litigation counsel.
          </p>
        </div>
        <div className="flex items-center gap-2 text-sm">
          <Badge variant={lit.referred ? "success" : "outline"}>
            {lit.referred ? `Referred (${lit.referredAt})` : "Not referred"}
          </Badge>
          <Badge variant={recommend ? "warning" : "outline"}>
            {recommend ? "Escalation recommended" : "Negotiation ongoing"}
          </Badge>
          <Badge
            variant={
              daysToSOL != null && daysToSOL <= 30 ? "destructive" : "outline"
            }
          >
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select applicable reasons and key dates.
          </CardDescription>
        </CardHeader>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div>
              ...
              <DatePicker
                id="sol"
                value={lit.solDate ?? ""}
                onChange={(v) => setLit((s) => ({ ...s, solDate: v }))}
              />
              <div className="text-xs text-muted-foreground mt-1">
                {daysToSOL == null ? "" : `${daysToSOL} day(s) remaining`}
              </div>
            </div>
            <div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Capture consent and any notes before referral.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <label className="inline-flex items-center gap-2 text-sm">
            <input
              type="checkbox"
              checked={lit.clientConsent}
              onChange={(e) =>
                setLit((s) => ({ ...s, clientConsent: e.target.checked }))
              }
            />
            I consent to referral for litigation if negotiations do not resolve
            my case.
          </label>
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setLit((s) => ({ ...s, clientNotes: e.target.value }))
              }
            />
          </div>
        </CardContent>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-3">
            <div>
              ...
              <Input
                ...
                value={lit.firmName ?? ""}
                onChange={(e) =>
                  setLit((s) => ({ ...s, firmName: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <Input
                id="attorney"
                value={lit.attorneyName ?? ""}
                onChange={(e) =>
                  setLit((s) => ({ ...s, attorneyName: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <Input
                id="email"
                type="email"
                value={lit.attorneyEmail ?? ""}
                onChange={(e) =>
                  setLit((s) => ({ ...s, attorneyEmail: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <Input
                id="phone"
                value={lit.attorneyPhone ?? ""}
                onChange={(e) =>
                  setLit((s) => ({ ...s, attorneyPhone: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <Input
                ...
                value={lit.venue ?? ""}
                onChange={(e) =>
                  setLit((s) => ({ ...s, venue: e.target.value }))
                }
              />
            </div>
            <div className="sm:col-span-2">
              ...
              <Input
                ...
                value={lit.defendants ?? ""}
                onChange={(e) =>
                  setLit((s) => ({ ...s, defendants: e.target.value }))
                }
              />
            </div>
          </div>
          ...
          <div className="flex justify-end">
            <Button
              ...
              disabled={!lit.clientConsent || lit.referred}
            >
              Mark Referred
            </Button>
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Snapshot of current case status for counsel.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 text-sm">
          <div className="rounded-md border border-card-border p-3">
            ...
            <div className="font-medium">
              {intakeStep >= 6 ? "Submitted" : "In progress"}
            </div>
          </div>
          <div className="rounded-md border border-card-border p-3">
          ...
          <div className="rounded-md border border-card-border p-3">
            ...
            <div className="font-medium">
              {lit.targetAmount ? `$${lit.targetAmount.toLocaleString()}` : "-"}
            </div>
          </div>
          <div className="rounded-md border border-card-border p-3">
            <div className="text-muted-foreground">SOL</div>
            <div className="font-medium">
              {lit.solDate || "-"} {daysToSOL != null ? `(${daysToSOL}d)` : ""}
            </div>
          </div>
        </CardContent>
      </Card>
