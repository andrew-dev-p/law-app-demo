...
export default function DemandReviewPage() {
  ...
  const prereqs = useMemo(() => {
    ...
    const anyBills =
      providers.some((p) => p.billsReceived) ||
      (intake?.uploads?.length ?? 0) > 0;
    return {
      intakeSubmitted,
      anyRecords,
      anyBills,
    ...
    <div className="w-full p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Prepare, review, and approve the demand to send to the insurer.
          </p>
        </div>
        <div className="flex items-center gap-2 text-sm">
          <Badge variant={demand.draftReady ? "success" : "outline"}>
            Draft {demand.draftReady ? "Ready" : "Pending"}
          </Badge>
          <Badge variant={demand.approved ? "success" : "outline"}>
            Approved {demand.approved ? "Yes" : "No"}
          </Badge>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            These help ensure the demand is well-supported.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-2 text-sm">
          <div className="flex items-center justify-between rounded-md border border-card-border px-3 py-2">
            ...
            <Badge
              variant={prereqs.intakeSubmitted ? "success" : "destructive"}
            >
              {prereqs.intakeSubmitted ? "Yes" : "No"}
            </Badge>
          </div>
          <div className="flex items-center justify-between rounded-md border border-card-border px-3 py-2">
            <span>Any medical records received</span>
            <Badge variant={prereqs.anyRecords ? "success" : "destructive"}>
              {prereqs.anyRecords ? "Yes" : "No"}
            </Badge>
          </div>
          <div className="flex items-center justify-between rounded-md border border-card-border px-3 py-2">
            <span>Any bills or documents on file</span>
            <Badge variant={prereqs.anyBills ? "success" : "destructive"}>
              {prereqs.anyBills ? "Yes" : "No"}
            </Badge>
          </div>
          <div className="flex justify-end pt-2">
            {!demand.draftReady ? (
              <Button onClick={generateDraft} disabled={!prereqs.ready}>
                Generate Draft
              </Button>
            ) : !demand.approved ? (
      ...
      {demand.draftReady && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Summary for demo purposes, not legally binding.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            <div className="rounded-md border border-card-border p-3">
              ...
              <div className="text-muted-foreground mt-1">
                Claimant: {intake?.personal?.firstName}{" "}
                {intake?.personal?.lastName || ""}. Incident type:{" "}
                {intake?.incident?.type || "-"} on{" "}
                {intake?.incident?.date || "-"}. Treatment ongoing. Providers:{" "}
                {providers.length || 0}. Attached docs:{" "}
